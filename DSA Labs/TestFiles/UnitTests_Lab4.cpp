...
int UnitTests_Lab4::Battery_InsertWithIndex() {
	...
	failVec.push_back(InsertWithIndex_MoreThanOneValueAdded);
...
FailResult UnitTests_Lab4::InsertWithIndex_ValueNotAdded() {
	...
	for (size_t i = 0; i < randomSize; ++i)
		vec.push_back(RandomFloat(1, 100));
	int randomIndex = RandomInt(3, randomSize - 1);
	float randomVal = RandomFloat(101, 200);
	DSA_Lab4 list;
	list.mList = std::list<float>(vec.begin(), vec.end());
...
FailResult UnitTests_Lab4::InsertWithIndex_MoreThanOneValueAdded() {
	int randomSize = RandomInt(5, 10);
	std::vector<float> vec;
	for (size_t i = 0; i < randomSize; ++i)
		vec.push_back(RandomFloat(1, 100));
	...
	FailResult result;
	result.check = list.mList.size() > vec.size()+1;
	result.deduction = 10;
	result.msg = "More than one value was added.";
