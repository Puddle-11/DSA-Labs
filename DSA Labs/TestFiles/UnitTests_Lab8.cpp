...
#ifdef _DEBUG
#pragma comment(lib, "HuffmanLibD.lib")
#else
#pragma comment(lib, "HuffmanLibR.lib")
#endif
...
#ifndef _GRADING
	results::Log("Huffman.log", { "Huffman.h" });
	// results::WriteResults(std::vector<const char*>{ "Huffman.h" }, "Huffman.results");
#endif
	int totalScore = 0;
...
#if HUFFMAN_DEBUG_COMPRESS
	totalScore += Battery_DebugCompress();
#endif
#if HUFFMAN_DEBUG_DECOMPRESS
	totalScore += Battery_DebugDecompress();
#endif
...
FailResult UnitTests_Lab8::Fail_Compress_DoesNotPassHeader() {
	bool passesFreqTable = SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",(const char*)mFrequencyTable") ||
		...
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",reinterpret_cast<char*>(mFrequencyTable)");
...
FailResult UnitTests_Lab8::Fail_Compress_OpensIncorrectFileForWriting() {
	bool opensOutputFileWithBitIfstream = SearchFile("Huffman.h", "void Compress(", "void Decompress(", "_outputFile, (const char*)") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "_outputFile, reinterpret_cast<const char*>") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "_outputFile, (char*)") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "_outputFile, reinterpret_cast<char*>"
				   );
...
#if HUFFMAN_DEBUG_COMPRESS
int UnitTests_Lab8::Battery_DebugCompress() {
	randomFile.name = "helloworld.original";
	CreateDuplicateFile("Files\\helloworld.txt", randomFile.name.c_str());

	FailVector failVec;

	failVec.push_back(Fail_DebugCompress_DoesNotPassHeader);
	failVec.push_back(Fail_DebugCompress_UsesSizeofForHeader);
	failVec.push_back(Fail_DebugCompress_DoesNotPassCorrectSizeForHeader);
	failVec.push_back(Fail_DebugCompress_OpensIncorrectFileForReading);
	failVec.push_back(Fail_DebugCompress_OpensIncorrectFileForWriting);
	failVec.push_back(Fail_DebugCompress_CompressedFileIsIncorrect);

	PassVector passVec;
	passVec.push_back(Pass_DebugCompress_CompressedFileIsCorrect);

	UnitTestBattery("Testing Compress (Debug)", 0, failVec, passVec);
...
#pragma region Fail Tests
FailResult UnitTests_Lab8::Fail_DebugCompress_DoesNotPassHeader() {
	bool passesFreqTable = SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",(const char*)mFrequencyTable") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",(char*)mFrequencyTable") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",reinterpret_cast<const char*>(mFrequencyTable)");
...
FailResult UnitTests_Lab8::Fail_DebugCompress_UsesSizeofForHeader() {
	bool usesSizeof = SearchFile("Huffman.h", "void Compress(", "void Decompress(", "sizeof(mFrequencyTable)");
...
FailResult UnitTests_Lab8::Fail_DebugCompress_DoesNotPassCorrectSizeForHeader() {
	bool usesCorrectHeaderSize = SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",1024") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",sizeof(int)*256") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",256*sizeof(int)") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", ",sizeof(unsigned int)*256") ||
...
FailResult UnitTests_Lab8::Fail_DebugCompress_OpensIncorrectFileForReading() {
	bool opensInputFileWithIfstream = SearchFile("Huffman.h", "void Compress(", "void Decompress(", "mFileName, std::ios::binary") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "mFileName, std::ios_base::binary") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "mFileName.c_str(), std::ios::binary") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "mFileName.c_str(), std::ios_base::binary") ||
...
FailResult UnitTests_Lab8::Fail_DebugCompress_OpensIncorrectFileForWriting() {
	bool opensOutputFileWithBitIfstream = SearchFile("Huffman.h", "void Compress(", "void Decompress(", "_outputFile, (const char*)") ||
		SearchFile("Huffman.h", "void Compress(", "void Decompress(", "_outputFile, reinterpret_cast<const char*>");
...
FailResult UnitTests_Lab8::Fail_DebugCompress_CompressedFileIsIncorrect() {
	std::string compressedFilename = randomFile.name;
	...
	CreateDuplicateFile("Files\\helloworld.huff", checkFilename.c_str());
	Huffman huff(randomFile.name);
	huff.Compress(compressedFilename.c_str());
...
#pragma region Pass Tests
bool UnitTests_Lab8::Pass_DebugCompress_CompressedFileIsCorrect() {
	std::string compressedFilename = randomFile.name;
	size_t extPos = compressedFilename.find_last_of('.');
	compressedFilename.erase(extPos + 1, compressedFilename.length());
	compressedFilename += "student";
...
#if HUFFMAN_DEBUG_DECOMPRESS
int UnitTests_Lab8::Battery_DebugDecompress() {
	std::cout << "\tNext test will take a few seconds\n";
	...
	compressedFilename.erase(extPos + 1, compressedFilename.length());
	compressedFilename += "compressed";
	CreateDuplicateFile("Files\\helloworld.huff", compressedFilename.c_str());
	//HuffmanCompress(randomFile.name.c_str(), compressedFilename.c_str());


	FailVector failVec;
	failVec.push_back(Fail_DebugDecompress_DoesNotPassHeader);
	failVec.push_back(Fail_DebugDecompress_UsesSizeofForHeader);
	failVec.push_back(Fail_DebugDecompress_DoesNotPassCorrectSizeForHeader);
	failVec.push_back(Fail_DebugDecompress_UsesEofForIfstream);
	failVec.push_back(Fail_DebugDecompress_OpensIncorrectFileForWriting);
	failVec.push_back(Fail_DebugDecompress_DecompressedFileIsTooLarge);
	failVec.push_back(Fail_DebugDecompress_DecompressedFileIsIncorrect);

	PassVector passVec;
	passVec.push_back(Pass_DebugDecompress_DecompressedFileIsCorrect);

	UnitTestBattery("Testing Decompress (Debug)", 0, failVec, passVec);
...
#pragma region Fail Tests
FailResult UnitTests_Lab8::Fail_DebugDecompress_DoesNotPassHeader() {
	bool passesFreqTable = SearchFile("Huffman.h", "void Decompress(", "};", ",(char*)mFrequencyTable") ||
		SearchFile("Huffman.h", "void Decompress(", "};", ",reinterpret_cast<char*>(mFrequencyTable)");
...
FailResult UnitTests_Lab8::Fail_DebugDecompress_UsesSizeofForHeader() {
	bool usesSizeof = SearchFile("Huffman.h", "void Decompress(", "};", "sizeof(mFrequencyTable)");
...
FailResult UnitTests_Lab8::Fail_DebugDecompress_UsesEofForIfstream() {
	bool usesEof = SearchFile("Huffman.h", "void Decompress(", "};", ".eof()");
...
FailResult UnitTests_Lab8::Fail_DebugDecompress_DoesNotPassCorrectSizeForHeader() {
	bool usesCorrectHeaderSize = SearchFile("Huffman.h", "void Decompress(", "};", ",1024") ||
		SearchFile("Huffman.h", "void Decompress(", "};", ",sizeof(int)*256") ||
		SearchFile("Huffman.h", "void Decompress(", "};", ",256*sizeof(int)") ||
		SearchFile("Huffman.h", "void Decompress(", "};", ",sizeof(unsigned int)*256") ||
...
FailResult UnitTests_Lab8::Fail_DebugDecompress_OpensIncorrectFileForWriting() {
	bool opensInputFileWithIfstream = SearchFile("Huffman.h", "void Decompress(", "};", "_outputFile, std::ios::binary") ||
		SearchFile("Huffman.h", "void Decompress(", "};", "_outputFile, std::ios_base::binary") ||
		SearchFile("Huffman.h", "void Decompress(", "};", "_outputFile, std::ofstream::binary");
...
FailResult UnitTests_Lab8::Fail_DebugDecompress_DecompressedFileIsTooLarge() {
	std::string uncompressedFilename = randomFile.name;
	size_t extPos = uncompressedFilename.find_last_of('.');
	uncompressedFilename.erase(extPos + 1, uncompressedFilename.length());
	uncompressedFilename += "student";
...
FailResult UnitTests_Lab8::Fail_DebugDecompress_DecompressedFileIsIncorrect() {
	std::string uncompressedFilename = randomFile.name;
	size_t extPos = uncompressedFilename.find_last_of('.');
	uncompressedFilename.erase(extPos + 1, uncompressedFilename.length());
	uncompressedFilename += "student";
...
#pragma region Pass Tests
bool UnitTests_Lab8::Pass_DebugDecompress_DecompressedFileIsCorrect() {
	std::string uncompressedFilename = randomFile.name;
	size_t extPos = uncompressedFilename.find_last_of('.');
	uncompressedFilename.erase(extPos + 1, uncompressedFilename.length());
	uncompressedFilename += "student";
