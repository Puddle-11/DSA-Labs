...
#ifndef _GRADING
	results::Log("Dictionary.log", { "Dictionary.h" });
...
FailResult UnitTests_Lab5::Fail_DictionaryConstructor_TableContainsIncorrectNumberOfLists() {
	...
	int check = sizeof(std::list<Pair>);
...
int UnitTests_Lab5::Battery_Clear() {
	...
	failVec.push_back(Fail_Clear_MemoryIsAllocated);
...
FailResult UnitTests_Lab5::Fail_Clear_MemoryIsAllocated() {
	bool allocatesMemory = SearchFile("Dictionary.h", "void Clear()", "void Insert", "=new");

	FailResult result;
	result.check = allocatesMemory == true;
	result.deduction = 10;
	result.msg = "There should be no dynamic allocations";
...
FailResult UnitTests_Lab5::Fail_Insert_OverwriteExistingKey_MemoryHasBeenAllocated() {
	...
	result.deduction = 10;
	result.msg = "No memory should be manually allocated and no new pairs should be added";
